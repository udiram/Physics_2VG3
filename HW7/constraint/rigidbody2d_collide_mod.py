...
def collideCylinderPolygon( scene, pCyl, pPoly, sign ):
    ...
    for i in range(nvertices):
        x1 = vp[i]
        x2 = vp[(i+1)%nvertices]
        xcl = collidePointLine(scene, xc, x1, x2)
        xcontacts.append(xcl)
        if (xcl == None): continue
        dcen = (xcl-xc).length()
        if (dcen < pCyl.radius):
            markerAdd(scene, xcl, "collision")
            contacts.append( ( pCyl.radius-dcen, xcl, -(xcl-xc).normalized()*sign ) )#was sign error

    for i in range(nvertices):
        xv = vp[i]  
        if (xcontacts[i]!=None or xcontacts[(i+1)%nvertices]!=None): 
            continue
        rcc = xv-xc
