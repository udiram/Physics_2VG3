...
class Spring20Demo(ShowBase):
    def __init__(self):
        ...
        # For equal masses m=1 and separation r:
        # k(r-L) = m v^2 / (r/2)  =>  v = sqrt(k(r-L)r/2)
        self.required_speed = math.sqrt(
            self.spring_k * (self.target_sep - self.spring_length) * self.target_sep / 2.0
        )
        ...
        # Masses spin about fixed COM at (0, 100, 0).
        self.particles = []
        ...
        OnscreenText(
            text=f"spring_20.py  |  L={self.spring_length:.1f}, k={self.spring_k:.1f}, target separation={self.target_sep:.1f}",
            pos=(-1.3, 0.92),
            scale=0.05,
            fg=(1, 1, 1, 1),
        ...
        OnscreenText(
            text=f"Required initial speed per mass: v = {self.required_speed:.4f}",
            pos=(-1.3, 0.85),
            scale=0.05,
            fg=(1, 1, 1, 1),
            align=TextNode.ALeft,
        ...
        print(f"Initial velocity magnitude for final steady separation 20: v = {self.required_speed:.6f}")
