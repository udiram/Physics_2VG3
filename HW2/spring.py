...
class SimpleScene(ShowBase):
    ...
    def updateParticles(self, task):
        ...
        r = self.particles[1].getPos() - self.particles[0].getPos()
        F = r.normalized() * (r.length() - self.springLength) * self.springK
        self.particles[0].force = F
        self.particles[1].force = -F
