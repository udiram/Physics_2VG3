...
def vec2_avg(points: list[tuple[float, float]]) -> tuple[float, float]:
    ...
    return (sum_x * inv_count, sum_z * inv_count)
...
def square_vertices_xz(x: Vec3, radius: float, angle: float) -> list[tuple[float, float]]:
    angle_degrees = angle * 180.0 / 3.141592653589793238462
    square = [
        Vec3(-1.0, 0.0, -1.0),
        Vec3(-1.0, 0.0, 1.0),
        Vec3(1.0, 0.0, 1.0),
    ...
    rotation = Mat3().rotateMatNormaxis(angle_degrees, Vec3(0.0, 1.0, 0.0))
    vertices = []
...
def markerClean(scene):
    if hasattr(scene, "markers"):
        previous_markers = scene.markers
        scene.markers = []
